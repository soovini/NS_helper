package kr.ac.incheon.ns.ns.utility;

import android.content.Context;
import android.content.SharedPreferences;
import android.content.SharedPreferences.Editor;

public class Preference {

    /** 디바이스 화면 정보 */
    public final static String SQUARE_SIZE = "SQUARE_SIZE";
    public final static String DP10 = "DP10";

    /** 로그인 정보 */
    public final static String USER_NO      = "USER_NO";
    public final static String USER_ID      = "USER_ID";
    public final static String USER_NAME    = "USER_NAME";
    public final static String USER_PHONE   = "USER_PHONE";
    public final static String USER_TYPE    = "USER_TYPE";
    public final static String USER_IMG     = "USER_IMG";

    private static SharedPreferences mSingletonPreferences;

    private static SharedPreferences getSharedPreferences(Context context) {
        if (mSingletonPreferences == null) {
            synchronized (Preference.class) {
                if (mSingletonPreferences == null) {
                    mSingletonPreferences = context.getSharedPreferences(context.getPackageName(), Context.MODE_PRIVATE);
                }
            }
        }
        return mSingletonPreferences;
    }

    public static void shutdown() {
        mSingletonPreferences = null;
    }

    public static void clearPreferences(Context context) {
        Editor editor = getSharedPreferences(context).edit();
        editor.clear();
        editor.apply();
        shutdown();
    }

    public static boolean getValue(Context context, int resId, boolean defaultValue) {
        return getSharedPreferences(context).getBoolean(context.getString(resId), defaultValue);
    }

    public static boolean getValue(Context context, String prefKey, boolean defaultValue) {
        return getSharedPreferences(context).getBoolean(prefKey, defaultValue);
    }

    public static int getValue(Context context, int resId, int defaultValue) {
        return getSharedPreferences(context).getInt(context.getString(resId), defaultValue);
    }

    public static int getValue(Context context, String prefKey, int defaultValue) {
        return getSharedPreferences(context).getInt(prefKey, defaultValue);
    }

    public static String getValue(Context context, int resId, String defaultValue) {
        return getSharedPreferences(context).getString(context.getString(resId), defaultValue);
    }

    public static String getValue(Context context, String prefKey, String defaultValue) {
        return getSharedPreferences(context).getString(prefKey, defaultValue);
    }

    public static void put(Context context, int resId, int newValue) {
        Editor editor = getSharedPreferences(context).edit();
        editor.putInt(context.getString(resId), newValue);
        editor.apply();
    }

    public static void put(Context context, String prefKey, int newValue) {
        Editor editor = getSharedPreferences(context).edit();
        editor.putInt(prefKey, newValue);
        editor.apply();
    }

    public static void put(Context context, int resId, String newValue) {
        Editor editor = getSharedPreferences(context).edit();
        editor.putString(context.getString(resId), newValue);
        editor.apply();
    }

    public static void put(Context context, String prefKey, String newValue) {
        Editor editor = getSharedPreferences(context).edit();
        editor.putString(prefKey, newValue);
        editor.apply();
    }

    public static void put(Context context, int resId, Boolean newValue) {
        Editor editor = getSharedPreferences(context).edit();
        editor.putBoolean(context.getString(resId), newValue);
        editor.apply();
    }

    public static void put(Context context, String prefKey, Boolean newValue) {
        Editor editor = getSharedPreferences(context).edit();
        editor.putBoolean(prefKey, newValue);
        editor.apply();
    }

    public static void put(Context context, int resId) {
        Editor editor = getSharedPreferences(context).edit();
        editor.remove(context.getString(resId));
        editor.apply();
    }

    public static void removePreference(Context context, String prefKey) {
        Editor editor = getSharedPreferences(context).edit();
        editor.remove(prefKey);
        editor.apply();
    }

}
